...
import { TimeTokenContract, MemoTokenContract, MimTokenContract, wMemoTokenContract, AirdropContract } from "../../abi";
...
export const loadAccountDetails = createAsyncThunk("account/loadAccountDetails", async ({ networkID, provider, address }: ILoadAccountDetails): Promise<IUserAccountDetails> => {
    ...
    if (addresses.AIRDROP_ADDRESS) {
        const airdropContract = new ethers.Contract(addresses.AIRDROP_ADDRESS, AirdropContract, provider);
        const airdropBalancesInfo = await airdropContract.status(address);
        claimableBalance = airdropBalancesInfo.claimableAmount;
        claimedAmount = airdropBalancesInfo.claimedAmount;
        pendingBalance = airdropBalancesInfo.maxPayout;
        vestingTerm = airdropBalancesInfo.endEpoch;
    }
    ...
    return {
        ...
        airdrop: {
            claimable: ethers.utils.formatUnits(claimableBalance, "gwei"),
            claimed: ethers.utils.formatUnits(claimedAmount, "gwei"),
            pending: ethers.utils.formatUnits(pendingBalance, "gwei"),
            vesting: vestingTerm.toString(),
        },
    };
});
...
const initialState: IAccountSlice = {
    ...
    airdrop: { claimable: "", claimed: "", pending: "", vesting: "" },
